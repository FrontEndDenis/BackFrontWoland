...
PARAMS = -f docker/docker-compose.yml --env-file docker/.env
...
ifeq ($(CMD_ARGUMENTS),)
	...
	docker-compose ${PARAMS} -p $(PROJECT_NAME)_$(HOST_UID) run --rm $(SERVICE_TARGET) sh
else
	# run the command
	docker-compose ${PARAMS} -p $(PROJECT_NAME)_$(HOST_UID) run --rm $(SERVICE_TARGET) sh -c "$(CMD_ARGUMENTS)"
endif
...
rebuild:
	...
	docker-compose ${PARAMS} build --no-cache $(SERVICE_TARGET)

build:
	# only build the container. Note, docker does this also if you apply other targets.
	docker-compose ${PARAMS} build
pull:
	docker-compose ${PARAMS} pull $(SERVICE_TARGET)
clean:
	# remove created images
	@docker-compose ${PARAMS} -p $(PROJECT_NAME)_$(HOST_UID) down --remove-orphans --rmi all 2>/dev/null \
...
test:
	...
	docker-compose ${PARAMS} -p $(PROJECT_NAME)_$(HOST_UID) run --rm $(SERVICE_TARGET) sh -c '\
...
js-dep:
	docker-compose ${PARAMS} run app npm i
js-prod:
	docker-compose ${PARAMS} run app npm run prod
js-dev:
	docker-compose ${PARAMS} run app npm run dev
js-watch:
	docker-compose ${PARAMS} run app npm run dev
db-up:
	docker-compose ${PARAMS} up -d mariadb
